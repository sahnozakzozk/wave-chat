  ...
  return (
    <div style={{ ...styles.app, ...(theme === "light" ? LIGHT_THEME_VARS : {}) }} className="wave-app-fullscreen">
      <div style={styles.rail}>
        ...
        >
          <Inbox size={18} />
          {incomingRequests.length > 0 && <span style={styles.railUnreadDot} />}
        </button>
      </div>
        ...
        {activeSection === "server" ? (
          <>
            <div style={styles.channelPanelHeader}>
              <span style={{ fontWeight: 800 }}>{showArchived ? s.archivedSection : activeServer.name}</span>
              <button
                onClick={() => {
                  setShowArchived((v) => !v);
                  setMenuOpenFor(null);
                }}
                style={{ ...styles.headerSearchBtn, marginLeft: "auto", color: showArchived ? "var(--accent)" : "var(--muted)" }}
                className="wave-btn"
                      ...
                      >
                        ...
                        <div style={styles.channelItemBody}>
                          <div style={styles.channelItemTitleRow}>
                            <span style={styles.channelItemName}>{c.name}</span>
          ...
          <>
            <div style={styles.channelPanelHeader}>
              <span style={{ fontWeight: 800 }}>{showArchived ? s.archivedSection : t.directMessages}</span>
              <button
                onClick={() => {
                  setShowArchived((v) => !v);
                  setMenuOpenFor(null);
                  ...
                  return (
                    <div key={u} style={{ position: "relative" }}>
                      <div
                        ...
                        onKeyDown={(e) => e.key === "Enter" && selectDmPartner(u)}
                        style={{ ...styles.channelItem, ...(u === activeDmPartner ? styles.channelItemActive : {}) }}
                        className="wave-btn"
                      >
                        <div style={{ ...styles.channelItemAvatar, background: colorForName(u) }}>{initials(u)}</div>
                        <div style={styles.channelItemBody}>
                          <div style={styles.channelItemTitleRow}>
                          ...
                          <div style={styles.channelItemPreviewRow}>
                            ...
                            {(unreadCounts[dmId] || 0) > 0 && u !== activeDmPartner && (
                              <span style={styles.unreadBadge}>{unreadCounts[dmId]}</span>
                            )}
                          </div>
                        </div>
                        <button
        ...
        {currentChannelType === "voice" && currentChannelId ? (
          ...
          <>
            ...
            <form onSubmit={handleSend} style={styles.composer}>
              ...
              />
              <button type="submit" style={styles.sendButton} className="wave-btn wave-send-btn" aria-label={t.send}>
                <Send size={16} />
              </button>
            </form>
            )}
...
function FindFriendsModal({ s, username, directory, friends, onWrite, onSendRequest, onClose }) {
              ...
              {results.map((u) => {
                ...
                return (
                  <div key={u} style={{ ...settingsStyles.row, cursor: "default" }}>
                    <div style={{ ...settingsStyles.rowIcon, background: colorForName(u) }}>{initials(u)}</div>
                    <span style={settingsStyles.rowText}>
...
function ChatItemMenu({ s, showRename, showDelete, showSubscribe, isSubscribed, isArchived, onArchive, onRename, onDelete, onSubscribeToggle, onClose }) {
  return (
    <div style={styles.chatItemMenu} onClick={(e) => e.stopPropagation()}>
      {showSubscribe && (
        <button
          onClick={() => {
            onSubscribeToggle();
...
const styles = {
  ...
  bubbleOwn: {
    background: "linear-gradient(135deg, #F2A93B, #E0793D)",
    color: "#241405",
  },
  bubbleOther: {
    background: "var(--elevated)",
