...
function getLoginAttempts(name) {
  try {
    const raw = localStorage.getItem("wave:loginAttempts:" + name);
...
function setLoginAttempts(name, data) {
  try {
    localStorage.setItem("wave:loginAttempts:" + name, JSON.stringify(data));
  } catch {}
}

function clearLoginAttempts(name) {
  try {
    localStorage.removeItem("wave:loginAttempts:" + name);
  } catch {}
}
...
function LoginScreen({ onLogin, t, lang, onToggleLang }) {
  ...
  async function handleSubmit(e) {
    ...
    if (mode === "login") {
      const attempts = getLoginAttempts(name);
      if (attempts.lockedUntil && Date.now() < attempts.lockedUntil) {
        const secondsLeft = Math.ceil((attempts.lockedUntil - Date.now()) / 1000);
        setError(
          lang === "ru"
            ? `Слишком много попыток. Попробуй через ${secondsLeft} сек.`
    ...
    try {
      ...
      if (mode === "register") {
        ...
        // Support legacy unsalted accounts created before this update.
        const salt = typeof record === "string" ? "" : record.salt;
        const storedHash = typeof record === "string" ? record : record.hash;
        const hashed = await hashPassword(password, salt);
        if (storedHash !== hashed) {
          const attempts = getLoginAttempts(name);
          const nextCount = (attempts.count || 0) + 1;
          if (nextCount >= 5) {
            setLoginAttempts(name, { count: 0, lockedUntil: Date.now() + 5 * 60 * 1000 });
            setError(
              lang === "ru"
                ? "Слишком много попыток. Попробуй снова через 5 минут."
                : "Too many attempts. Try again in 5 minutes."
            );
          } else {
            setLoginAttempts(name, { count: nextCount, lockedUntil: 0 });
            setError(t.wrongPassword);
          }
          setBusy(false);
          return;
        }
        // Upgrade legacy unsalted accounts to a salted hash on successful login.
        if (typeof record === "string") {
          const newSalt = makeSalt();
          const newHash = await hashPassword(password, newSalt);
          users[name] = { hash: newHash, salt: newSalt };
