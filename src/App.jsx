...
const STRINGS = {
  ru: {
    ...
    loginHint: "Вход запоминается на этом устройстве. Пароль не нужен — только код с почты.",
  ...
  en: {
    ...
    loginHint: "Your session is remembered on this device. No password needed — just the code from your email.",
...
async function verifyEmailCode(email, code) {
  try {
    ...
    return { ok: true, userId: data.user && data.user.id };
  } catch (err) {
    return { ok: false, error: "network" };
  }
}
...
// Three-step login: email -> 6-digit code (sent by Supabase Auth) -> pick a
// username (only shown the very first time a given email is seen). Returning
// people skip straight from a verified code to their saved username.
...
function LoginScreen({ onLogin, t, lang, onToggleLang }) {
  ...
  async function handleVerifyCode(e) {
    ...
    // Code is correct — the email is genuinely theirs. Now figure out
    // whether this is a returning person (has a saved username already)
    // or a first-timer who still needs to pick one.
  ...
  return (
    <div style={styles.loginWrap} className="wave-app-fullscreen">
      <div style={styles.loginCard} className={error ? "wave-shake" : ""}>
        ...
        <p style={styles.loginSubtitle}>{step === "email" ? t.tagline : step === "code" ? t.codeSentTo(email) : t.chooseUsernameHint}</p>
