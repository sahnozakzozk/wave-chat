...
const STRINGS = {
  ru: {
    ...
    messageActionsTitle: "Действие с сообщением",
    replyingTo: "Ответ",
    photoLabel: "Фото",
...
const SESSION_TTL_MS = 7 * 24 * 60 * 60 * 1000; // stay logged in for 7 days

async function persistSession(name) {
  try {
    await storage.set("session", JSON.stringify({ name, ts: Date.now() }), false);
  } catch (err) {
    // session persistence is best-effort
  }
}

// Reads the saved session and returns the username only if it's still
// within the 7-day window; otherwise clears it and returns null so the
// person falls back to the login screen.
async function readValidSession() {
  const result = await storage.get("session", false).catch(() => null);
  ...
  try {
    ...
    // legacy sessions saved before this change stored a bare username
    // string instead of JSON — treat those as freshly issued so existing
    // logged-in users aren't booted out immediately after this update.
    name = result.value;
    ts = Date.now();
  }
  if (!name || typeof ts !== "number" || Date.now() - ts > SESSION_TTL_MS) {
    await storage.delete("session", false).catch(() => {});
    return null;
  }
  return name;
}
...
class WaveErrorBoundary extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidCatch(error, info) {
    this.setState({ info });
    // Also surface it in the console for anyone with dev tools open.
    // eslint-disable-next-line no-console
    console.error("Wave crashed:", error, info);
  }
  render() {
    if (this.state.error) {
      return (
        ...
        >
          <h2 style={{ color: "#F09595", margin: "0 0 20px" }}>Ошибка</h2>
          <button
...
function AppInner() {
  ...
  // Interactive edge-swipe-back gesture (like iOS): dragging from the chat
  // screen's left edge follows the finger and reveals the chat list
  // underneath, instead of the list/chat switch being an instant hard cut.
  const [edgeDrag, setEdgeDrag] = useState(null); // { startX, startY, dx, active }
  const chatPanelRef = useRef(null);
  const edgeDragRef = useRef(null); // mirrors edgeDrag for use inside touch handlers without stale closures
  function handleChatTouchStart(e) {
    if (mobilePanel !== "chat" || e.touches.length !== 1) return;
    const touch = e.touches[0];
    if (touch.clientX > 28) return; // only the left edge starts the gesture
    const state = { startX: touch.clientX, startY: touch.clientY, dx: 0, active: false };
  ...
  function handleChatTouchMove(e) {
    ...
    if (!state.active) {
      // Only "commit" to the swipe once horizontal intent is clear, so
      // normal vertical scrolling of the message list is never hijacked.
  ...
  function handleChatTouchEnd() {
    ...
    const width = chatPanelRef.current ? chatPanelRef.current.offsetWidth : window.innerWidth;
    const progress = width > 0 ? state.dx / width : 0;
    setEdgeDrag(null);
    if (progress > 0.35) setMobilePanel("list");
  }
  ...
  // Custom in-app replacements for window.confirm()/alert() — the browser's
  // native dialogs don't match the app's own design, so every confirmation
  // and notice is rendered as an on-brand modal/snackbar instead.
  const [confirmDialog, setConfirmDialog] = useState(null); // { message, danger, confirmLabel, resolve }
  const [notice, setNotice] = useState(null); // { id, message }
  const noticeTimerRef = useRef(null);
  function askConfirm(message, opts) {
    return new Promise((resolve) => {
      setConfirmDialog({ message, danger: opts?.danger !== false, confirmLabel: opts?.confirmLabel, resolve });
    });
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [sessionName, savedLang] = await Promise.all([
          readValidSession(),
          loadLang(),
        ]);
        if (!cancelled) {
  ...
  // Moved up from below the `if (!username) return <LoginScreen .../>` early
  // return — a hook can never live after a conditional return, since that
  // makes the number of hooks called differ between the "logged out" and
  // "logged in" renders of this same component (React error #310).
  useEffect(() => {
    return () => {
  ...
  async function handleDeleteChannel(channelId) {
    if (!(await askConfirm(s.deleteChannelConfirm))) return;
    const next = await deleteCustomChannel(activeServerId, channelId);
    if (!next) {
      showNotice(t.saveFailed);
      return;
    }
    setCustomChannels((prev) => ({ ...prev, [activeServerId]: next }));
  ...
  async function handleDeleteDm(dmId) {
    if (!(await askConfirm(s.deleteDmConfirm))) return;
    ...
    if (results.some((r) => r === null)) showNotice(t.saveFailed);
    setMenuOpenFor(null);
  }
  ...
  async function handleBulkDelete() {
    if (selectedIds.length === 0) return;
    if (!(await askConfirm(s.bulkDeleteConfirm))) return;
  ...
  // Popular chat gestures on message bubbles: double-tap for a quick heart
  // reaction, press-and-hold for an action sheet, and swipe-right to reply
  // — all mutually exclusive so one gesture never misfires as another.
  const tapRef = useRef({ id: null, time: 0 });
  const bubbleGestureRef = useRef(null); // { id, startX, startY, dx, active }
  const longPressTimerRef = useRef(null);
  const longPressFiredRef = useRef(false);
  const [messageActionsFor, setMessageActionsFor] = useState(null);
  const [swipeReply, setSwipeReply] = useState(null); // { id, dx }
  const [copiedMsgId, setCopiedMsgId] = useState(null);

  function handleBubbleTouchStart(e, m) {
    if (e.touches.length !== 1) return;
    const touch = e.touches[0];
    bubbleGestureRef.current = { id: m.id, startX: touch.clientX, startY: touch.clientY, dx: 0, active: false };
    longPressFiredRef.current = false;
    clearTimeout(longPressTimerRef.current);
    longPressTimerRef.current = setTimeout(() => {
      const state = bubbleGestureRef.current;
      if (state && state.id === m.id && !state.active) {
        longPressFiredRef.current = true;
        bubbleGestureRef.current = null;
        setSwipeReply(null);
        setMessageActionsFor(m);
        if (navigator.vibrate) navigator.vibrate(8);
      }
    }, 450);
  }
  function handleBubbleTouchMove(e, m) {
    const state = bubbleGestureRef.current;
    if (!state || state.id !== m.id) return;
    const touch = e.touches[0];
    const dx = touch.clientX - state.startX;
    const dy = touch.clientY - state.startY;
    if (!state.active) {
      ...
      if (Math.abs(dy) > Math.abs(dx)) {
        // Vertical intent — leave it to normal scrolling.
        bubbleGestureRef.current = null;
        clearTimeout(longPressTimerRef.current);
        return;
      }
    ...
    state.dx = Math.max(0, Math.min(64, dx));
    setSwipeReply({ id: m.id, dx: state.dx });
  }
  function handleBubbleTouchEnd(m) {
    clearTimeout(longPressTimerRef.current);
    ...
    bubbleGestureRef.current = null;
    if (state && state.active) {
      if (state.dx > 40) setReplyingTo(m);
      setSwipeReply(null);
      return;
    }
    setSwipeReply(null);
    const now = Date.now();
    if (tapRef.current.id === m.id && now - tapRef.current.time < 300) {
      toggleReaction(m, "❤️");
      tapRef.current = { id: null, time: 0 };
    } else {
      tapRef.current = { id: m.id, time: now };
    }
  }
  function handleBubbleTouchCancel() {
    clearTimeout(longPressTimerRef.current);
    longPressFiredRef.current = false;
    bubbleGestureRef.current = null;
    setSwipeReply(null);
  }
  function handleBubbleDoubleClick(m) {
    toggleReaction(m, "❤️");
  }
  async function copyMessageText(m) {
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        setCopiedMsgId(m.id);
        setTimeout(() => setCopiedMsgId((id) => (id === m.id ? null : id)), 1200);
      }
    } catch (err) {
  ...
  // 0 = list fully shown, 1 = chat fully shown. While an edge-swipe-back
  // drag is active it follows the finger instead of jumping instantly, so
  // switching between the two screens never feels like a hard "wall".
  const edgeProgress = edgeDrag && edgeDrag.active && chatPanelRef.current ? Math.min(1, edgeDrag.dx / chatPanelRef.current.offsetWidth) : 0;
  const panelProgress = mobilePanel === "chat" ? 1 - edgeProgress : 0;
  const panelTransition = edgeDrag && edgeDrag.active ? "none" : "transform 0.32s cubic-bezier(0.32, 0.72, 0, 1)";
  const scrimTransition = edgeDrag && edgeDrag.active ? "none" : "opacity 0.32s cubic-bezier(0.32, 0.72, 0, 1)";

  return (
    <div style={{ ...styles.app, ...(theme === "light" ? LIGHT_THEME_VARS : {}) }} className="wave-app-fullscreen">
      <div
        style={{ ...styles.channelPanel, transform: `translateX(${-30 * panelProgress}%)`, transition: panelTransition }}
      ...
      >
        <div style={styles.folderTabsBar}>
          ...
          <button
            onClick={() => showNotice(t.demoNoServer)}
      ...
      <div
        ...
        className="wave-mobile-scrim"
      />

      <div
        ref={chatPanelRef}
        style={{ ...styles.chatPanel, transform: `translateX(${(1 - panelProgress) * 100}%)`, transition: panelTransition }}
        className="wave-chat-panel"
                    ...
                    <div style={{ minWidth: 0, maxWidth: "78%", display: "flex", flexDirection: "column", alignItems: isOwn ? "flex-end" : "flex-start" }}>
                      ...
                      {m.type === "sticker" ? (
                        <div
                          style={{
                            position: "relative",
                            transform: swipeReply && swipeReply.id === m.id ? `translateX(${swipeReply.dx}px)` : "none",
                            transition: swipeReply && swipeReply.id === m.id ? "none" : "transform 0.2s ease",
                          }}
                          ...
                          onDoubleClick={() => handleBubbleDoubleClick(m)}
                        >
                          {swipeReply && swipeReply.id === m.id && (
                            <CornerUpLeft
                              size={14}
                              style={{ position: "absolute", left: -26, top: "50%", transform: "translateY(-50%)", opacity: Math.min(1, swipeReply.dx / 40), color: "var(--muted)" }}
                            />
                          )}
                        ...
                        >
                          ...
                          {copiedMsgId === m.id && (
                            <span style={styles.copiedBadge}>{t.copiedMessage}</span>
                          )}
                        </div>
                      )}
      ...
      {notice && (
        <div style={styles.noticeWrap}>
          <div key={notice.id} style={styles.noticeCard}>{notice.message}</div>
        </div>
      )}

      {messageActionsFor && (
        <div style={styles.actionSheetOverlay} onClick={() => setMessageActionsFor(null)}>
          <div style={styles.actionSheetCard} onClick={(e) => e.stopPropagation()}>
            ...
            <button
              ...
              onClick={() => {
                setReplyingTo(messageActionsFor);
                setMessageActionsFor(null);
              }}
            >
              <CornerUpLeft size={17} style={{ opacity: 0.75 }} /> {t.reply}
            </button>
            <button
              className="wave-btn"
              style={styles.actionSheetRow}
              onClick={() => {
                setReactionPickerFor(messageActionsFor.id);
                setMessageActionsFor(null);
              }}
            >
              <Smile size={17} style={{ opacity: 0.75 }} /> {t.addReaction}
            </button>
            {messageActionsFor.type === "text" && (
              <button className="wave-btn" style={styles.actionSheetRow} onClick={() => copyMessageText(messageActionsFor)}>
                <Copy size={17} style={{ opacity: 0.75 }} /> {t.copyMessage}
              </button>
            )}
          </div>
        </div>
      ...
      {confirmDialog && (
        <div style={styles.confirmOverlay} onClick={() => resolveConfirm(false)}>
          <div style={styles.confirmCard} onClick={(e) => e.stopPropagation()}>
            <div style={{ ...styles.confirmIcon, ...(confirmDialog.danger ? styles.confirmIconDanger : {}) }}>
              <Trash2 size={20} />
            </div>
            <p style={styles.confirmMessage}>{confirmDialog.message}</p>
            <div style={styles.confirmActions}>
              <button onClick={() => resolveConfirm(false)} className="wave-btn" style={styles.confirmCancelBtn}>
                {s.cancel}
              </button>
              <button
              ...
              >
                {confirmDialog.confirmLabel || s.delete}
              </button>
            </div>
          </div>
        </div>
...
@keyframes wave-sheet-up { from { opacity: 0; transform: translateY(100%); } to { opacity: 1; transform: translateY(0); } }
...
@media (max-width: 767.98px) {
  .wave-channel-panel, .wave-chat-panel { position: absolute !important; inset: 0; width: 100% !important; }
}
@media (min-width: 768px) {
  .wave-channel-panel, .wave-chat-panel { display: flex !important; position: relative !important; transform: none !important; inset: auto; }
  ...
  .wave-mobile-scrim { display: none !important; }
}
`;
...
const styles = {
  ...
  copiedBadge: {
    ...
    color: "var(--text)",
    whiteSpace: "nowrap",
    animation: "wave-toast-in 0.15s ease",
    pointerEvents: "none",
  },
  actionSheetOverlay: {
    position: "absolute",
  ...
  actionSheetCard: {
    ...
    animation: "wave-sheet-up 0.28s cubic-bezier(0.32, 0.72, 0, 1)",
  },
  actionSheetHandle: {
    width: 36,
    height: 4,
