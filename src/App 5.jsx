...
async function createCustomChannel(serverId, name, type) {
  try {
    const existing = await loadCustomChannels(serverId);
    const id = `${serverId}-custom-${Date.now()}`;
    const next = [...existing, { id, name, type: type || "text" }];
...
export default function App() {
  ...
  async function handleCreateChannel(name, type) {
    const next = await createCustomChannel(activeServerId, name, type);
    if (next) {
  ...
  return (
    <div style={{ ...styles.app, ...(theme === "light" ? LIGHT_THEME_VARS : {}) }}>
          ...
          <>
            <div style={styles.channelPanelHeader}>
              ...
              <button
                ...
                style={{ ...styles.headerSearchBtn, marginLeft: "auto" }}
                className="wave-btn"
            ...
            <div style={styles.channelList}>
              ...
              {(customChannels[activeServerId] || []).map((c) => (
                <button
                  key={c.id}
                  onClick={() => selectChannel(c.id)}
                  style={{ ...styles.channelItem, ...(c.id === activeChannelId ? styles.channelItemActive : {}) }}
        ...
        <div style={styles.userFooter}>
          ...
          <div style={styles.avatarWrap}>
            <div
              style={{
                ...styles.avatar,
                background: profilePhoto ? "transparent" : colorForName(username),
                backgroundImage: profilePhoto ? `url(${profilePhoto})` : undefined,
                backgroundSize: "cover",
                backgroundPosition: "center",
              }}
            >
              {!profilePhoto && initials(username)}
            </div>
            ...
            <div style={settingsStyles.scroll}>
              <div style={settingsStyles.profileCard}>
                <div style={settingsStyles.bigAvatarWrap}>
                  <div
                    style={{
                      ...settingsStyles.bigAvatar,
                      background: localPhoto ? "transparent" : accent,
                      backgroundImage: localPhoto ? `url(${localPhoto})` : undefined,
                      backgroundSize: "cover",
                      backgroundPosition: "center",
                    }}
                  >
                    {!localPhoto && initialsText}
                  </div>
      ...
      <div style={settingsStyles.page}>
        ...
        <div style={settingsStyles.scroll}>
          ...
          <div style={{ display: "flex", gap: 8, margin: "4px 0 20px" }}>
            ...
            >
              <Hash size={14} /> {s.channelTypeText}
            </button>
            <button
